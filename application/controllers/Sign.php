...
class Sign extends CI_Controller{
	...
	*/
	public function signup()
	{
        ...
        if(!$this->_validate_signup($email,$username,$password,$captcha))
        {
        	return false;
        }
	...
	*/
	public function signin()
	{
		...
		$validation = $this->_validate_signin($signin_username,$signin_password,$signin_captcha);
		if (!$validation)
		{
			return ;
		}
		...
		//检查登录错误次数
		$fail_num = $this->_get_fail_count($signin_username);
		if ($fail_num >= MAX_ERR_NUM)
		{
			$this->_set_fail_count($signin_username);
			$ttl = $this->_get_ttl($signin_username);
			$ttl = ceil($ttl/60);
		...
		if ($signin_password == $user->password)
		{
			...
			$this->_reset_fail_count($signin_username);
		...
		{
			$this->_set_fail_count($signin_username);
			$count = 5 - $this->_get_fail_count($signin_username);
	...
	*/
	public function reset_passwd($reset_flag)
	{
		...
		if (!$this->_check_password_formate($passwd))
		{
			$this->cg_base->echo_json(-1,'fail');
			return;
		}
	...
	* @return bool
	*/
	private function _validate_signup($email,$username,$password,$captcha)
	{
		if (empty($email))
		{
		...
		if (!$this->_check_email_formate($email)) 
		{
			$this->cg_base->echo_json(-9,$this->error_msg[-9]);
			return false;
		}
		if(!$this->_validate_signin($username,$password,$captcha))
		{
			return false;
		}
		//检查用户名是否已经注册
	...
	* @return bool
	*/
	private function _validate_signin($username,$password,$captcha)
	{
		if (empty($username))
		{
			$this->cg_base->echo_json(-1,$this->error_msg[-1]);
		...
		if (!$this->_check_username_formate($username))
		{
			$this->cg_base->echo_json(-3,$this->error_msg[-3]);
			return false;
		}
		if (!$this->_check_password_formate($password))
		{
	...
	/**
	* _check_username_formate
	* 检查用户名格式
	*
	* @return bool
	*/
	private function _check_username_formate($username)
	{
		$res = preg_match('/^[0-9A-Za-z_]{6,32}$/', $username);
		return $res ? true:false;
	}
	/**
	* _check_email_formate
	* 检查邮件格式
	*
	* @return bool
	*/
	private function _check_email_formate($email)
	{
	...
	* @return bool
	*/
	private function _check_password_formate($password)
	{
	...
	* @return bool
	*/
	private function _check_signin()
	{
		//redis
		if (isset($_COOKIE['uid'])) {
	...
	* @return void
	*/
	private function _set_fail_count($username)
	{
	...
	* @return int
	*/
	private function _get_fail_count($username)
	{
		$key = md5($username.'sign_fail');
		if ($this->redis->exists($key))
		{
	...
	* @return bool
	*/
	private function _reset_fail_count($username)
	{
		$key = md5($username.'sign_fail');
		if ($this->redis->exists($key))
		{
	...
	* @return int  
	*/
	private function _get_ttl($username)
	{
		$key = md5($username.'sign_fail');
		if ($this->redis->exists($key))
		{
